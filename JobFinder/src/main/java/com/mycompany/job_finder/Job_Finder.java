...
public class Job_Finder extends JFrame {
    ...
    private JPanel createAddJobPanel() {
        ...
        addFormField(formPanel, "Job Title:", titleField, gbc, 0);
    ...
    private JPanel createSearchPanel() {
        ...
        searchButton.addActionListener(e -> searchJobs(searchField.getText()));
    ...
    private void searchJobs(String searchTerm) {
    ...
    private void showJobDetails(int selectedRow, boolean isOwnJob) {
        ...
        titleField.setEditable(isOwnJob);
        ...
        // Add components with labels
        addDetailField(detailsPanel, "Title:", titleField, gbc, 0);
        ...
        if (isOwnJob) {
            ...
            saveButton.addActionListener(e -> {
                updateJob(jobId, titleField.getText(), companyField.getText(), 
    ...
    private void updateJob(int jobId, String title, String company, String location, 
        ...
        String sql = "UPDATE jobs SET title=?, company=?, location=?, website=?, email=?, description=?, tags=? WHERE id=? AND user_id=?";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
    ...
    private void loadAllJobs() {
        ...
        String sql = "SELECT * FROM jobs ORDER BY CASE WHEN user_id = ? THEN 0 ELSE 1 END, id DESC";
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
    ...
    private void addJob() {
        ...
        try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
            ...
            pstmt.setString(8, tagsField.getText().replace(',', ';')); // Fixing the addJob method to properly handle tags
